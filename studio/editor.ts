...
import './assets/ws-icons.css'

interface Editor extends Vue, EditorScope { }
...
const ediorId = parseInt(url.parse(window.location.href, true).query.id, 10)
...

{
    ...
    for (const key in ediorState) {
        ...
        stateGetters[key] = () => ($state as any)[key]
    }

    _.assign($state, defaultWeb, ipcRenderer.sendSync('editor-property', ediorId, syncKeys))

    Vue.mixin({
        computed: stateGetters,
        methods: editorActions
    })
